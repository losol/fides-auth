// heartbeat.ts
//
// Framework-agnostic, browser-side session keepalive engine. This is the logic
// behind the React `useHeartbeat` hook, extracted so it has no React dependency:
// any adapter (React, React Router, plain JS) can start it and get a `stop()`
// back. It uses browser globals (`fetch`, `document`, `window`) at runtime, but
// importing this module performs no I/O and touches no globals.
...
export interface HeartbeatConfig {
   ...
   * the engine waits for the next interaction (or tab focus) and refreshes then.
   * @default 1_800_000 (30 minutes)
   */
  idleThresholdMs?: number;

  /**
   * Known access-token expiry at start (ISO 8601 string or epoch ms). When
  ...
  /**
   * Called on transient errors (network failure, 5xx). The engine retries — this
...
/** Handle returned by {@link createHeartbeat}. */
export interface HeartbeatHandle {
  /** Stops the heartbeat: clears timers, removes listeners, aborts in-flight fetches. */
  stop: () => void;
}
...
/**
 * Starts an expiry-driven session keepalive and returns a handle to stop it.
 *
 ...
 * (skew floor). It primes with one refresh on start to learn the current expiry
 ...
 *
 * No-ops (returns a handle whose `stop` does nothing) when there is no `window`,
...
export function createHeartbeat(config: HeartbeatConfig = {}): HeartbeatHandle {
  ...
  async function beat(): Promise<void> {
    ...
    try {
      ...
      // Stopped (or 401-driven teardown) during the fetch — bail before touching
      // callbacks so we don't fire on a dead consumer.
      if (stopped) return;
